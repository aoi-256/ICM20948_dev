...
void ICM20948::AK09916_getdata(float* mag_buffer){
	...
	if((magdata_status[0] & 0x01) == 0){
		...
		mag_z[0]  = (float)((int16_t)(data_buffer[19] << 8 | data_buffer[18]));

		//取得した値が有効である場合
		if(mag_x[0] < 1000.0 && mag_x[0] > -1000.0 && mag_x[0] != 0){

			//平均化によるLFPと出力
			if(mag_x[5] == 0){

				//データがそろっていないときは取得値をそのまま出力
				mag_buffer[0] = mag_x[0];
				mag_buffer[1] = mag_y[0];
				mag_buffer[2] = mag_z[0];
			}
			else{

				mag_buffer[0] = (mag_x[0]*5 + mag_x[1]*4 + mag_x[2]*3 + mag_x[3]*2 + mag_x[4]*1) / 15.0;
				mag_buffer[1] = (mag_y[0]*5 + mag_y[1]*4 + mag_y[2]*3 + mag_y[3]*2 + mag_y[4]*1) / 15.0;
				mag_buffer[2] = (mag_z[0]*5 + mag_z[1]*4 + mag_z[2]*3 + mag_z[3]*2 + mag_z[4]*1) / 15.0;
			}

			//値が更新されていた場合は、その値を保存する
			if(mag_x[1] != mag_x[0] && mag_y[1] != mag_y[0] && mag_z[1] != mag_z[0]){
				...
				//取得した値が有効でない場合は1つ前の値を返す
				mag_buffer[0] = mag_x[1];
				mag_buffer[1] = mag_y[1];
				mag_buffer[2] = mag_z[1];
			}
		}
	}
	else{

		//センサーの準備ができていなかった時は１つ前の値を返す
		mag_buffer[0] = mag_x[1];
		mag_buffer[1] = mag_y[1];
		mag_buffer[2] = mag_z[1];
	}
